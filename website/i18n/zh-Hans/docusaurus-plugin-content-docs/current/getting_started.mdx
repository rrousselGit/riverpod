...
当你确定好你想要安装的包文件后，你可以通过下方的单行命令来为你的应用添加依赖包：
...
<AutoSnippet title="终端" {...pubadd}></AutoSnippet>

</TabItem>

<TabItem value="riverpod">

<AutoSnippet title="终端" {...dartPubadd}></AutoSnippet>

</TabItem>

</Tabs>

或者手动添加下列代码到你的 `pubspec.yaml` 文件中：
...
<ConditionalSnippet codegen={true}>
  最后，用如下命令来运行代码生成工具 <code>flutter pub run build_runner watch</code>
</ConditionalSnippet>
...
<ConditionalSnippet codegen={true}>
  最后，用如下命令来运行代码生成工具 <code>dart pub run build_runner watch</code>
</ConditionalSnippet>

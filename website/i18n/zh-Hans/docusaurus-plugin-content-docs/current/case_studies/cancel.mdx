...
然后，让我们定义我们的详细信息页面。
要获取活动并实施下拉刷新，
请参阅<Link documentID="case_studies/pull_to_refresh" />案例研究。

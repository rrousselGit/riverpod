...
a single activity:
-->
现在，我们要定义一个提供者发出 HTTP GET 请求来获取单个活动：
...
在那里，我们可以使用 `ref.refresh` 刷新我们选择的提供程序。
...
-->
为了获得这样的 future，我们可以读取提供者的 `.future` 属性。
这将返回一个 future，该 future 在我们的提供者解决后完成。

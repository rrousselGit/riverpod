...
-->
首先，我们应该声明新的提供者/通知者：这需要一些思维过程，这取决于您独特的业务逻辑。
...
-->
事实上，Riverpod 的通知程序一次可以暴露*一个*实体。
...
-->
初始化通知程序很简单：只需在 `build` 内编写初始化逻辑即可。  
我们现在可以摆脱旧的 `_init` 函数。
...
-->
Riverpod 将通过公开 `AsyncValue<List<Todo>>` 自动转换任何异步提供程序，

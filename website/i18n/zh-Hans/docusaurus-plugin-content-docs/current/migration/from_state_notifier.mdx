...
-->
- `StateNotifier` 的反应式依赖项在其提供程序中声明，而 `Notifier`
  将此逻辑集中在其 `build` 方法中
- `StateNotifier` 的整个初始化过程分为其提供者和构造函数，
...
- 无论您正在编写哪种类型的提供程序，语法现在都是统一的，从而支持代码生成
...
此外，与其他提供程序不同的是，`dispose` 回调将在类中单独定义。  
...
-->
`StateNotifier` 的 `dispose` 方法指的是通知程序本身的 dispose 事件，
也就是*在自行处置之前*调用的回调。
...
在新的通知程序中，处置生命周期仅在_一个_地方处理，通过 `ref.onDispose`
（和其他），就像任何其他提供程序一样。
...
我们现在无法了解我们的通知程序在执行 `update` 时是否仍然存在。  
...
属性可能会误导新通知程序：`mounted` *几乎总是* `true` 。
...
-->
消费者可以使用相同的语法从这三个提供者获取数据，
这在您从 `StateNotifier` 迁移时非常有用；这也适用于通知程序方法。
...
-->
这是有意为之，因为我们希望与 `Notifier`、`AsyncNotifier` 和其他提供程序实现完全的 API 统一。  
事实上，使用 `Notifier` 或 `AsyncNotifier` 应该与任何其他提供程序没有任何不同。
...
它们应该通过使用其提供者进行交互：如果不这样做将会*破坏*通知程序，

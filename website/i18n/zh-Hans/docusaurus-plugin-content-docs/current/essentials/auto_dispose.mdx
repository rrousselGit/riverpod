...
-->
使用代码生成时，默认情况下，当提供程序停止被侦听时，状态将被销毁。  
当侦听器在整个帧中没有活动的侦听器时，会发生这种情况。
当这种情况发生时，状态将被销毁。
...
-->
不使用代码生成时，默认情况下，当提供程序停止侦听时，状态不会被销毁。
...
-->
您可以选择更改此行为并使用自动处置。
执行此操作时，Riverpod 将跟踪提供商是否有侦听器。
然后，如果一帧的时间内提供程序没有侦听器，则状态将被销毁。

<!---
To enable automatic disposal, you can use `.autoDispose` next to the provider type:
-->
若要启用自动处置，可以在提供程序类型旁边使用 `.autoDispose`：
...
-->
启用/禁用自动处置在重新计算提供程序时，对于是否销毁状态没有影响。  
重新计算提供程序时，状态将始终被销毁。
:::
...
-->
- 提供程序不再使用，并且处于“自动释放”模式（稍后会详细介绍）。
  在这种情况下，与提供程序的所有关联状态都将被销毁。
- 提供程序将重新计算，例如 `ref.watch`。
  在这种情况下，将释放以前的状态，并创建一个新状态。
...
-->
- `ref.onCancel` 当删除提供程序的最后一个侦听器时调用。
- `ref.onResume` 当 `onCancel` 调用之后添加新的侦听器时调用。
...
-->
提供程序可以使用 `ref.invalidateSelf` 使自己失效。
...
-->
如上所述，当自动处置启用时，如果在完整的一帧时间里提供程序没有侦听器，状态将被销毁。
...
-->
可以调整此逻辑以满足您的需求。
例如，仅当提供程序在特定时间内未被侦听时，
才可以使用 `ref.onCancel`/`ref.onResume` 销毁状态。

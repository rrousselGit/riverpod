...
-->
您可能已经注意到 `ref` 有两种方法可以强制提供程序重新计算，并且想知道它们有何不同。
...
-->
`yourMethod` 将使用 `Notifier` 的 `Ref` 与其他提供者交互。
...
-->
- 无法使用 `InheritedWidget` 实现侦听器的“当更改时”。
...
- 监听 `InheritedWidget` 的小部件永远不会停止监听它。
  ...
  对于业务逻辑来说，这是一个问题。
  假设您使用提供程序来表示分页 API。
  当页面偏移量发生变化时，您不希望小部件继续侦听先前可见的页面。
...
-->
最后且同样重要的一点是，您可以通过重写通知程序上的方法 `updateShouldNotify` 来更改此行为。
...
通过 `ref.watch` 依赖于 "user" 提供者的用户状态。
...
-->
然后，当用户注销时，依赖于它的所有提供程序将自动重置，但其他所有内容都将保持不变。

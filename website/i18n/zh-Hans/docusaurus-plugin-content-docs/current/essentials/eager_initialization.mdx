...
-->
默认情况下，所有提供程序都以延迟方式初始化。
这意味着提供程序仅在首次使用时进行初始化。
这对于仅在应用程序的某些部分使用的提供程序很有用。
...
-->
不幸的是，由于 Dart 的工作方式（出于摇树目的），没有办法将提供程序标记为需要急切初始化。
但是，一种解决方案是在应用程序的根组件下强制读取要急切初始化的提供程序。

<!---
The recommended approach is to simply "watch" a provider in a Consumer placed right under your `ProviderScope`:
-->
推荐的方法是简单地 "watch" 位于 `ProviderScope` 下方的 Consumer 中的提供者：
...
-->
不，事实并非如此。
在上面给出的示例中，负责急切初始化的消费者是一个单独的小部件，它只返回一个 `child` .
...
-->
因此，除非另一个小部件也在侦听该提供程序，否则只会 `_EagerInitialization` 重新构建。
...
您可以 `_EagerInitialization` 检查提供程序是否处于 "loading" 状态，
...
-->
#### 我已经处理了加载/错误状态，但其他消费者仍然收到 AsyncValue！有没有办法不必处理每个小部件中的加载/错误状态？
...
-->
与其试图让你的提供者不公开一个 `AsyncValue` ，不如让你的小部件使用 `AsyncValue.requireValue`。  

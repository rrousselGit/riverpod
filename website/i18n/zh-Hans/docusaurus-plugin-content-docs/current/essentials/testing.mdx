...
-->
现在我们有了 ProviderContainer，我们可以使用它来读取提供程序，使用：
...
-->
在自动释放提供程序时使用 `container.read` 时要小心。  
如果您的提供商没有被监听，其状态可能会在测试过程中被破坏。
...
无论如何，它的返回值都能读取提供程序的当前值，
同时还能确保提供程序不会在测试过程中被弃置：
...
或者，如果要与提供程序交互，可以获取 `ProviderContainer`。
...
-->
很酷的部分：默认情况下可以模拟所有提供程序，无需任何额外设置。  
...
-->
由于我们在测试中获得了一个 `ProviderContainer`，因此可以使用它来“监听”提供程序：
...
to be completed.
-->
在 Riverpod 中，提供程序返回 Future/Stream 是很常见的。  

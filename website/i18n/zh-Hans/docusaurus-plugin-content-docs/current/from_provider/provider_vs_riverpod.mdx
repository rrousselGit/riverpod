...
例如，状态通常在一个页面中创建，但在导航后稍后在另一个页面中销毁。  
...
-->
在 Riverpod 中，当一个提供者程序标记为 `.autoDispose` 在不再使用时会自动销毁的状态。  
当卸载最后一个删除提供者程序的小部件时，Riverpod 将检测到卸载并销毁提供者程序。  
...
ref.onDispose((){
  print("如果我已经被定义为 `.autoDispose`，我将被销毁！");
});
```

...
`FutureProvider` 可以是一种通过读取 JSON 文件创建的 `Configuration` 对象的便捷方法。

在 provider 内部使用 async/await 语法创建配置，再加上使用 Flutter 的资产系统即可。
...
这将在 [Future] 完成时自动重新构建 UI。同时，如果多个 widget 需要配置，Flutter 资产将只载入一次。

如你所见，在widget中监听 `FutureProvider` 会返回一个 [AsyncValue]，它允许你处理错误/加载中的状态。

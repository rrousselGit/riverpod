...
本文是为熟悉 [Provider] 并希望了解 Riverpod 的开发者所设计的。
...
一开始 Riverpod 被认为是解决这一问题的 Provider 的下一个大版本。
但最后并没有这么做，因为这样会是一个相当大的突破性变更，
而 Provider 是现如今最常用的 Flutter package 之一。
...
Riverpod 修复了 Provider 的许多基本问题，例如但不限于：

- 显著简化了与 provider 的组合。
  Riverpod 提供了简单强大的工具比如 [ref.watch] 和 [ref.listen] ，
...
所以要回答这个问题，你可能会问自己：
**我要使用 Provider 还是 Riverpod？**

你大概率会选择 Riverpod。
Riverpod 设计得更好还能急剧简化你的逻辑。

## Provider 和 Riverpod 的差异

### 定义 providers

两个 package 最主要的区别在于如何定于 provider。

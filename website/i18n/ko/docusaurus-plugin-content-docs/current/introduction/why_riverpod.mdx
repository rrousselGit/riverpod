...
선언적 프로그래밍과 반응형 프로그래밍을 사용하여, Riverpod은 당신을 위해 애플리케이션의 상단 부분을 처리합니다.
내장된 오류 처리 및 캐싱을 통해 네트워크 요청을 수행할 수 있으며, 
필요한 경우 데이터를 자동으로 데이터를 자동으로 다시 가져올 수 있습니다.
...
<AutoSnippet language="dart" 
    {...whyRiverpod}
    translations={{
        raw_fetchPackages: "// pub.dev에서 패키지 목록을 가져옵니다.",
        raw_fetchApi: "  // API를 호출합니다. 여기서는 package:dio를 사용하고 있지만 다른 것을 사용할 수도 있습니다.",
        raw_decodeJson: "  // JSON 응답을 Dart 클래스로 디코딩합니다.",
        codegen_fetchPackages: "// pub.dev에서 패키지 목록을 가져옵니다.",
        codegen_fetchApi: "  // API를 호출합니다. 여기서는 package:dio를 사용하고 있지만 다른 것을 사용할 수도 있습니다.",
        codegen_decodeJson: "  // JSON 응답을 Dart 클래스로 디코딩합니다.",
    }}
/>

...
<AutoSnippet
  ...
  translations={{
    ...
    ProviderScope: "    // 위젯이 프로바이더를 읽을 수 있게 하려면 전체 애플리케이션을 \"ProviderScope\" 위젯으로 감싸야 합니다.\n    // 여기에 프로바이더의 상태가 저장됩니다.",
    ConsumerWidget: "// 리버포드에 의해 노출되는 StatelessWidget 대신 ConsumerWidget을 확장합니다.",
    HookConsumerWidget: "// 리버포드에 의해 노출되는 HookWidget 대신 HookConsumerWidget을 확장합니다.",
...
<AutoSnippet
  ...
  translations={{
    helloWorld: "// 값을 저장할 \"provider\"를 생성합니다(여기서는 \"Hello world\").\n// 프로바이더를 사용하면 노출된 값을 모의(mock)//재정의(override)할 수 있습니다.",
    container: "  // 이 객체는 공급자의 상태가 저장되는 곳입니다.",
    value: "  // \"컨테이너\" 덕분에 공급자를 읽을 수 있습니다.",
  }}
/>

...
- 위젯 테스트: 일반적으로 Flutter 종속성이 있는 위젯 테스트.
  공급자를 사용하는 위젯의 동작을 테스트하는 데 유용할 수 있습니다.
...
<AutoSnippet 
  raw={createContainer} 
  translations={{
    raw_createContainer: "/// [ProviderContainer]를 생성하고 테스트가 끝나면\n/// 자동으로 폐기하는 테스트 유틸리티입니다.",
    raw_container: "  // ProviderContainer를 생성하고 선택적으로 매개변수 지정을 허용합니다.",
    raw_addTearDown: "  // 테스트가 끝나면 container를 폐기(dispose)합니다.",
  }}
/>
...
<AutoSnippet 
  raw={unitTest} 
  translations={{
    raw_container: "    // 이 테스트에 대한 프로바이더 컨테이너를 생성합니다.\n    // 테스트 간에 ProviderContainer를 공유하지 마세요.",
    raw_useProvider: "    // TODO: 컨테이너를 사용하여 애플리케이션을 테스트합니다.",
  }}
/>
...
<AutoSnippet 
  raw={fullWidgetTest} 
  translations={{
    raw_useProvider: "    // TODO: providers와 상호 작용"
  }}
/>
...
<AutoSnippet 
  {...providerToMock} 
  translations={{
    raw_provider: "// 이른 초기화된 공급자.",
    codegen_provider: "// 이른 초기화된 공급자.",
  }}
/>
...
<AutoSnippet 
  raw={mockProvider} 
  translations={{
    raw_container: "    // 단위 테스트에서는 이전의 \"createContainer\" 유틸리티를 재사용합니다.",
    raw_providers: "      // 모의(Mock)할 공급자 목록을 지정할 수 있습니다:",
    raw_exampleProvider: "        // 이 경우 \"exampleProvider\"를 모방(Mock)하고 있습니다.",
    raw_note: "          // 이 함수는 공급자의 일반적인 초기화 함수입니다.\n          // 일반적으로 \"ref.watch\"를 호출하고 초기 상태를 반환하는 곳입니다.\n\n          // 기본값인 \"Hello world\"를 사용자 정의 값으로 바꿔보겠습니다.\n          // 그러면 `exampleProvider`와 상호작용하면 이 값이 반환됩니다.",
    raw_providerScope: "    // ProviderScope를 사용하여 위젯 테스트에서도 동일한 작업을 수행할 수 있습니다:",
    raw_overrides: "        // ProviderScope에는 정확히 동일한 \"overrides\" 매개 변수가 있습니다.",
    raw_sameAsBefore: "          // 이전과 동일",
  }}
/>
...
<AutoSnippet 
  raw={awaitFuture} 
  translations={{
    raw_note: "    // TODO: 컨테이너를 사용하여 애플리케이션을 테스트합니다.\n    // 기대(expectation)가 비동기적이므로 \"expectLater\"를 사용해야 합니다.",
    raw_read: "      // \"provider\" 대신 \"provider.future\"를 읽습니다.\n      // 이는 비동기 공급자에서 가능하며, 공급자의 값으로 해결(resolve)될 future를 반환합니다.",
    raw_completion: "      // future가 예상한 값으로 resolve되는지 확인할 수 있습니다.\n      // 또는 오류에 \"throwsA\"를 사용할 수 있습니다.",
  }}
/>
...
왜냐하면 Notifiers는 자체적으로 인스턴스화할 수 없으며 프로바이더의 일부로 사용될 때만 작동하기 때문입니다.  
...
<AutoSnippet 
  {...notifierMock} 
  translations={{
    raw_mock: "// 모의클래스(Mock)는 notifier가 사용하는 것에 해당하는\n// Notifier base-class를 서브클래싱해야 합니다.",
    codegen_mock: "// 모의클래스(Mock)는 notifier가 사용하는 것에 해당하는\n// Notifier base-class를 서브클래싱해야 합니다.",
  }}
/>
...
<AutoSnippet 
  raw={notifierUsage} 
  translations={{
    raw_overrides: "      // provider를 Override하여 Mock Notifier를 생성하도록 합니다.",
    raw_readNotifier: "    // 그런 다음 컨테이너를 통해 Mock Notifier를 가져옵니다:",
    raw_interactNotifier: "    // 그러면 실제 notifier와 마찬가지로 notifier와 상호 작용할 수 있습니다:",
  }}
/>
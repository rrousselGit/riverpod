...
프로바이더는 초강력(super-powered) 함수입니다.
...
따라서 공급자는 _GET_ 네트워크 요청에 가장 적합합니다. (_POST/etc_ 요청의 경우, <Link documentID="essentials/side_effects" /> 참조)
...
<AutoSnippet title="activity.dart" 
  {...activity} 
  translations={{
    raw_activity: "/// `GET /api/activity` 엔드포인트의 응답입니다.",
    raw_fromJson: "  /// JSON 객체를 [Activity] 인스턴스로 변환합니다.\n  /// 이렇게 하면 API 응답을 유형 안전하게 읽을 수 있습니다.",
    codegen_activity: "/// `GET /api/activity` 엔드포인트의 응답입니다.\n///\n/// `freezed`와 `json_serializable`을 사용하여 정의됩니다.",
    codegen_fromJson: "  /// JSON 객체를 [Activity] 인스턴스로 변환합니다.\n  /// 이렇게 하면 API 응답을 유형 안전하게 읽을 수 있습니다.",
  }}
/>
...
이 변수는 공급자와 상호 작용하는 데 사용되는 변수입니다.  
...
수정자(Modifiers)는 선택 사항이며, 타입에 안전한 방식으로 공급자의 동작을 조정하는 데 사용됩니다.

현재 두 가지 수정자를 사용할 수 있습니다:

- 공급자가 더 이상 사용되지 않을 때 캐시를 자동으로 지우는 `autoDispose`.  
...
다른 provider와 상호작용하는 데 사용되는 객체입니다.  
...
여기에 프로바이더의 로직을 배치합니다.
이 함수는 공급자를 처음 읽을 때 호출됩니다.  
...
모든 프로바이더는 `@riverpod` 또는 `@Riverpod()`로 어노테이션해야 합니다.
이 어노테이션은 전역 함수나 클래스에 배치할 수 있습니다.  
이 어노테이션을 통해 프로바이더를 구성할 수 있습니다.
...
<AutoSnippet title="provider.dart" 
  {...provider} 
  translations={{
    raw_response: "  // package:http를 사용하여 Bored API에서 임의의 activity를 가져옵니다.",
    raw_json: "  // 그런 다음 dart:convert를 사용하여 JSON 페이로드를 맵 데이터 구조로 디코딩합니다.",
    raw_fromJson: "  // 마지막으로 맵을 Activity 인스턴스로 변환합니다.",
    codegen_note: "// 코드 생성이 작동하는 데 필요합니다.",
    codegen_provider: "/// 그러면 이 함수의 결과를 캐시할\n/// `activityProvider`라는 프로바이더가 생성됩니다.",
    codegen_response: "  // package:http를 사용하여 Bored API에서 임의의 activity를 가져옵니다.",
    codegen_json: "  // 그런 다음 dart:convert를 사용하여 JSON 페이로드를 맵 데이터 구조로 디코딩합니다.",
    codegen_fromJson: "  // 마지막으로 맵을 Activity 인스턴스로 변환합니다.",
  }}
/>

이 코드조각에서는 UI가 임의의 액티비티를 가져오는 데 사용할 수 있는 `activityProvider`라는 이름의 공급자를 정의했습니다.
다음과 같은 것은 주목할 가치가 있습니다:

- 네트워크 요청은 UI가 provider를 한 번 이상 읽을 때까지 실행되지 않습니다.
- 이후 읽기는 네트워크 요청을 다시 실행하지 않고 이전에 가져온 활동을 반환합니다.
- UI가 이 공급자의 사용을 중단하면 캐시가 삭제됩니다.
  그런 다음 UI가 이 공급자를 다시 사용하면 새로운 네트워크 요청이 이루어집니다.
...
:::info
Provider는 "지연(lazy)"입니다. 공급자를 정의해도 네트워크 요청이 실행되지 않습니다.
대신 공급자를 처음 읽을 때 네트워크 요청이 실행됩니다.
:::
...
<AutoSnippet 
  {...consumerWidget} 
  translations={{
    raw_note: "/// \"StatelessWidget\" 대신 \"ConsumerWidget\"을 서브클래스화했습니다.\n/// 이는 \"StatelessWidget\"을 만들고 \"Consumer\"를 재조정하는 것과 같습니다.",
    raw_build: "  // 이제 \"build\"가 추가 매개변수 \"ref\"를 받는 방식에 주목하세요",
    raw_watch: "    // \"Consumer\"를 사용했을 때와 마찬가지로 위젯 내부에서 \"ref.watch\"를 사용할 수 있습니다.",
    raw_render: "    // 렌더링 로직은 동일하게 유지됩니다.",    
  }}
/>
...
<AutoSnippet 
  ...
  translations={{
    raw_homeWidget: "// ConsumerStatefulWidget을 확장합니다.\n// 이것은 \"Consumer\" + \"StatefulWidget\"과 동일합니다.",
    raw_homeState: "// \"State\" 대신 \"ConsumerState\"를 확장한 것을 볼 수 있습니다.\n// 이것은 \"ConsumerWidget\" 대 \"StatelessWidget\"과 동일한 원리를 사용합니다.",
    raw_listen: "    // 상태 수명 주기에도 \"ref\"에 액세스할 수 있습니다.\n    // 이를 통해 특정 공급자에 리스너를 추가하여 대화 상자/스낵바를 표시하는 등의 작업을 수행할 수 있습니다.",
    raw_todo: "      // TODO 스낵바/대화 상자 표시",
    raw_watch: "    // \"ref\"는 더 이상 매개변수로 전달되지 않고 대신 \"ConsumerState\"의 프로퍼티가 됩니다.\n    // 따라서 \"build\" 내에서 \"ref.watch\"를 계속 사용할 수 있습니다.",
  }}
/>
...
<AutoSnippet 
  {...hookConsumerWidget} 
  translations={{
    raw_homeWidget: "/// \"HookConsumerWidget\"을 서브클래스화했습니다.\n/// 이것은 \"StatelessWidget\" + \"Consumer\" + \"HookWidget\"을 함께 결합합니다.",
    raw_build: "  // 이제 \"build\"가 추가 매개변수를 받는 방식에 주목하세요: \"ref\"",
    raw_useState: "    // 위젯 내부에서 \"useState\"와 같은 훅을 사용할 수 있습니다.",
    raw_activity: "    // 공급자 읽기도 사용할 수도 있습니다.",
  }}
/>

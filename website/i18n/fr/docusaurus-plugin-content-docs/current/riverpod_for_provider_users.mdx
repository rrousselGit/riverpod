...
qui souhaitent veulent se familiariser avec Riverpod.
...
Par exemple, si un fournisseur a été défini comme :
...
- Riverpod ne s'appuie pas sur les types génériques. Il s'appuie plutôt sur la variable
  créée à l'aide de la définition du fournisseur.
...
Lorsque vous utilisez un provider, la manière officielle de combiner des fournisseurs
...
en expliquant [comment lire les fournisseurs dans les widgets](#lecture-de-providers-buildcontext).
En effet, les fournisseurs sont maintenant capables d'écouter d'autres fournisseurs de la même façon
que les widgets.

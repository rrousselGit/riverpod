...
ou le provider écouté a été forcé de se rafraîchir à l'aide de [ProviderContainer.refresh]/[context.refresh].
...
[Mon provider fait des mises à jour trop souvent, que puis-je faire ?](#my-provider-updates-too-often-what-can-i-do)
Mauvaise pratique pour appeler `read` ici
:::

### Comment tester un objet qui reçoit [read] comme paramètre de son constructeur ?

Si vous utilisez le modèle décrit dans [Puis-je lire un fournisseur sans l'écouter ?](#can-i-read-a-provider-without-listening-to-it), 

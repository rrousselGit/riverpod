...
- `mayHaveChanged` est appelé lorsque la dépendance d'un provider a changé, mais il n'est pas encore certain que la valeur calculée change.

:::info
Il est possible que mayHaveChanged soit appelé, sans que didUpdateProvider soit appelé, 
par exemple lorsqu'un Provider est recalculé mais renvoie une valeur == à la précédente.
:::

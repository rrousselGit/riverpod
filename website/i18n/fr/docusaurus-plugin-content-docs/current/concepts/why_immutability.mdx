...
On parle d'immuabilité lorsque tous les champs d'un `Objet` sont 'final' ou 'late final'.
Ils sont définis une seule fois lors de la construction.

L'immuabilité est souhaitable pour de nombreuses raisons différentes

- L'égalité de valeur plutôt que l'égalité de référence
- Raisonnement "local" sur une partie du code
  - Un morceau de code très éloigné ne peut pas obtenir une référence et changer l'objet qui se trouve en dessous de lui.
- Plus facile de raisonner pour les tâches asynchrones et parallèles
  - Un autre code ne peut pas modifier votre objet entre deux opérations
- Sécurité des API
  - Ce que vous passez dans une méthode ne peut pas être modifié par le appelé / l'appelant
...
puisqu'il comporte plusieurs ajouts intéressants qui vont au-delà de la simple fabrication d'objets immuables, notamment :

- A generated copyWith method
- Deep copy (copyWith sur les objets freezed imbriqués)
- Union types
...
Si vous appelez simplement une méthode qui fait muter un objet, la référence est la même.
:::
...
Pour utiliser ce code, n'oubliez pas d'importer `freezed_annotation`, ajoutez la directive part et exécutez [build_runner] pour générer les classes figées !

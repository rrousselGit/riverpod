...
Marcare un provider `autoDispose` aggiunge inoltre una proprietà extra su `ref`:`maintainState`.

La proprietà `maintainState` è booleana (`false` di default) e consente al provider 
...
Questo è indesiderato, siccome causerebbe la mancata disposizione di `firstProvider` .

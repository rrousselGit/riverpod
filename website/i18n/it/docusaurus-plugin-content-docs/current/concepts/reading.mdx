...
La soluzione più comune sarà quella di sostituire [StatelessWidget] con [ConsumerWidget] 
quando si crea un widget.

[ConsumerWidget] è sostanzialmente identico a [StatelessWidget], con la sola differenza che 
il suo metodo "build" riceve un parametro extra: l'oggetto "ref".
...
class HomeViewState extends ConsumerState<HomeView> {
  ...
  Widget build(BuildContext context) {
    // Possiamo anche usare "ref" per ascoltare un provider all'interno del metodo build
    final counter = ref.watch(counterProvider);
    return Text('$counter');
  }
}
...
Questa soluzione è specifica per chi usa [flutter_hooks]. Dato che [flutter_hooks] 
richiede l'estensione di [HookWidget] per funzionare, i widget che usano gli hooks 
non possono estendere [ConsumerWidget].

Una soluzione, che è resa possibile dal package [hooks_riverpod], è rimpiazzare [HookWidget] 
con [HookConsumerWidget]. [HookConsumerWidget] si comporta come un [ConsumerWidget] e un [HookWidget]. 
...
class HomeView extends HookConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // HookConsumerWidget consente di usare gli hooks all'interno del metodo build
...
Una soluzione finale per ottenere un "ref" dentro i widget è fare affidamento 
a [Consumer]/[HookConsumer].
...
Questi widgets possono essere un modo per ottenere un "ref" senza dover 
definire una classe. 
Un esempio potrebbe essere:
...
Scaffold(
  body: HookConsumer(
    builder: (context, ref, child) {
      // Come HookConsumerWidget, possiamo usare gli hooks dentro il builder
      final state = useState(0);
...
- ottenere il valore di un provider e ascoltarne i cambiamenti, in modo tale 
  ...
  che ha sottoscritto il valore. Ciò può essere fatto usando `ref.watch`.
- aggiungendo un "listener" ad un provider, per eseguire un'azione ogni tal volta 
  che il provider cambia. Ciò può essere fatto usando `ref.listen`.
- ottenere il valore di un provider ignorandone i cambiamenti. Questo è utile quando 
  abbiamo bisogno del valore di un provider in un evento come `onPressed`. 
...
final filteredTodoListProvider = Provider((ref) {
  ...
  switch (filter) {
    ...
    case FilterType.none:
      // restituisce la lista non filtrata dei todo
      return todos;
  }
});
```
...
```
Questo pezzo di codice mostra un widget che ascolta un provider che memorizza un conteggio. 
Se quel conteggio cambia, il widget si ricostruirà e la UI si aggiornerà per mostrare il nuovo
valore.
...
Widget build(BuildContext context, WidgetRef ref) {
  // usa "read" per ignorare gli aggiornamenti su un provider
...
Se si usa `ref.read`, quando il valore necessita di cambiare, devi andare in tutto il codice 

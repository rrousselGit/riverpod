...
Say you've just migrated `myChangeNotifierProvider` like so:
```dart
...
If your inner code is depending on pkg:Provider consume APIs, the following is more than enough to start with your new pkg:Riverpod's providers.
```dart
MultiProvider(
  providers: [
    ChangeNotifierProvider.value(value: ref.watch(myNotifier.notifier)),
  ]
)
```

...
- Indeed, `StateNotifier`'s initialization is split between its provider and its constructor, whereas
`Notifier` centralizes initialization in its `build` method
- As opposed to `StateNotifier`, no logic is ever written into a `Notifier`'s constructor or on its Provider.

## Better async support with `AsyncNotifier`
Previously, with `StateNotifier`, we would handle stateful asynchronous logic like so:
```dart
WIP
```

## Further reading
WIP these should be re-arranged as suggested

### Family attributes are exposed by their classes directly
...
`Notifier` (and `AsyncNotifier`), instead, have their own `.family` counterpart, which has a more

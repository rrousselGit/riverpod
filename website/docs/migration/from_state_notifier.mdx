...
- Syntax is now unified, no matter the type of provider you're writing, enabling
...
it's reccomended to work around this by canceling the asynchronous operation.
...
As explained [here](/docs/concepts/modifiers/auto_dispose#example-canceling-http-requests-when-no-longer-used),
a `CancelToken` can be used to cancel requests.
...
[as explained here](/docs/essentials/combining_requests#the-reflistenlistenself-methods).
...
The [testing guide](/docs/essentials/testing) fully explains how to use `ProviderContainer.read` to do so.

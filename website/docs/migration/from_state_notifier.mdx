...
`Notifier` and `AsyncNotifier` exist exactly to replace the legacy `StateNotifier`
(and `ChangeNotifier`), with the following advantages:
- Conciseness and a better DX overall
- Codegen support
- One less [external dependency](https://github.com/rrousselGit/state_notifier)

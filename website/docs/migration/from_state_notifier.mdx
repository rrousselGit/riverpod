...
Easier done than said, let's write some example simple logic with the old `StateNotifier`:
<AutoSnippet language="dart" {...buildInitOld}></AutoSnippet>

Now, with the new `Notifier` APIs, this translates to:
<AutoSnippet language="dart" {...buildInit}></AutoSnippet>

Before anything, the following `Notifier` design choices:
...
As anticipated, `StateNotifier` suffers its heavy syntax.  
Instead, `AsyncNotifer`, just like `Notifier`, brings a simple and uniform API.

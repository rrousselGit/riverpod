...
- **Do not** upgrade Riverpod.  
  ...
  The tool will not execute properly if you have already installed the version 1.0.0.  
...
let's see in details the changes necessary.

### Syntax unification

The version 1.0.0 of Riverpod focused on the unification of the syntax for
...
With the version 1.0.0, only one syntax remains: `ref.watch(provider)`. The
others were removed.
...
Alternatively you can use the new `StateProvider.state` to keep the old behaviour.

...
### Understanding Persisted State

By default, when using `@JsonPersist()` or `persist()`, restored cache data is emitted as `AsyncLoading(value: ...)` rather than `AsyncData`. This allows you to show cached data while fetching fresh data in the background.

### Approach 1: Default behavior (Use when() or value)

By default (`emitAsLoadingState: true`), you can inspect `state.value` or handle `loading:` using `when()`:

```dart
@riverpod
Future<List<Todo>> sortedTodos(Ref ref) async {
  final todosState = ref.watch(todosProvider);
  
  final todos = todosState.when(
    data: (data) => data,
    loading: () => todosState.value ?? [],
    error: (error, stack) => todosState.hasValue ? todosState.value! : throw error,
  );
  
  return List<Todo>.from(todos)..sort((a, b) => a.name.compareTo(b.name));
}
```

### Approach 2: Emit directly as AsyncData (`emitAsLoadingState: false`)

If you prefer to use `.future` on a persisted provider, pass `emitAsLoadingState: false` to `persist()`:
...
class Todos extends _$Todos {
  ...
  FutureOr<List<Todo>> build() async {
    // Restored data is emitted as AsyncData, allowing .future to resolve immediately
    persist(
      ref.watch(riverpodCacheProvider.future),
      key: 'todos_list',
...
@riverpod
Future<List<Todo>> sortedTodos(Ref ref) async {
  // .future resolves immediately with the restored AsyncData
...
:::caution
Using `emitAsLoadingState: false` causes the provider to skip the `loading` state when restoring cached data. Only use this if you want to treat persisted data as ready immediately.
:::

...
When using `@JsonPersist()`, persisted data is emitted as `AsyncLoading(value: ...)` rather than `AsyncData`. This allows you to show cached data while fetching fresh data in the background.

### Approach 1: Handle AsyncLoading with value (Use when())
...
If you prefer to use `.future`, you can promote persisted data to `AsyncData`:
...
class Todos extends _$Todos {
  ...
  FutureOr<List<Todo>> build() async {
    // Load persisted data and promote to AsyncData
    await persist(ref.watch(riverpodCacheProvider.future), key: 'todos_list');
    
    // Optionally, you can set the state to AsyncData after loading
    // state = AsyncData(persistedValue);
...
@riverpod
Future<List<Todo>> sortedTodos(Ref ref) async {
  // Now .future will work because state is AsyncData
...
- **Approach 1**: When you want to show cached data immediately while loading.
- **Approach 2**: When you only care about the final data state and want to use `.future`.

...
### Listing active providers in a container

`ProviderContainer` exposes `allProviders()`, which returns the active providers
mounted in that container and its parents.

This can be useful for debugging, testing, or applying the same operation to
all currently live providers:

<AutoSnippet raw={allProviders} />

You can optionally specify `family` to only list active providers of a family:

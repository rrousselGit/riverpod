...
Riverpod pauses providers that nothing listens to, and widgets that are not visible.
[Ref.watch] follows that: a paused provider stops watching its dependencies.

[Ref.listen] does not. Its listener keeps being called even while the provider is
paused. This is on purpose: listeners typically perform side-effects, and silently
dropping a side-effect is rarely what you want.

When you'd rather have a listener stop along with its provider (or its widget), pass
...
The listener then stops being called for as long as the provider is paused (or, inside a
widget, for as long as the widget is not visible). If events were emitted in the
meantime, the listener receives the last one upon resuming.

This is useful for side-effects that only make sense while the user is looking at the
screen, such as showing a snackbar or playing a sound. Rather than firing for a screen
nobody can see, the listener stays quiet and catches up on the latest value once the
screen is visible again.

Note that `pauseWhenInactive` only withholds notifications. It does not notify you when
the provider becomes paused, so it cannot be used to release a resource. Use
[Ref.onCancel]/[Ref.onResume] for that.
...
[Ref.onResume]: https://pub.dev/documentation/hooks_riverpod/latest/hooks_riverpod/Ref/onResume.html

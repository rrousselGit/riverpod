...
However, this operation fails when `AsyncValue` has no data (yet). 
...
This is particularly useful in a flutter widget where we want to display different widgets based on the current async state.
...
## Modifying an async state synchronously
...
## Modifying the async state asynchronously
In order to safely modify an `AsyncValue` of a state asynchronously, we can use the convenience method `.guard()`, which in turn creates an `AsyncValue` that is safe to read.

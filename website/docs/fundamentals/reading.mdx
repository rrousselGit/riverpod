...
In the [previous article](./provider), we talking about what is a "provider"
and how to create one.  
Now, we will see how to read a provider

For part, we will implement a simple "counter" application.
For this, we will start by creating a "counter" provider using [StateProvider]:
...
In our case, we are making a counter, so we want to create an `int` and initialise
...
Both snippets behaves the same: They build a [Text] that changes when the counter
updates.  
...
[RaisedButton] no-longer unnecesserily rebuild when the counter changes.

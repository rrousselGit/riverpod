...
Providers are the most important piece [Riverpod]. In short, you can think
...
- Providers allow easily and efficiently listen to a piece of state.  
...
- `Provider`, the provider that we decided to used.  
...
- A function that creates the shared state.  
  That function will always receive an object called `ref` as parameter. This object
...
On the other hand, a [StreamProvider]'s callback will be expected to return a [Stream].
...
final weatherProvider = FutureProvider((ref) async {
  // We use `ref.dependOn` to read another provider, and we pass it the provider
  // that we want to consume. Here: cityProvider
  final city = ref.dependOn(cityProvider).value;
...
final weatherProvider = Provider((ref) {
  final city = ref.dependOn(cityProvider).value;
  final country = ref.dependOn(countryProvider).value;
...
final weatherProvider = Provider((ref) {
  ...
  // `Location` will then be able to call `ref.dependOn` to read the providers.
  return Location(ref);
});
...
class Location {
  ...
  String get label {
    final city = _ref.dependOn(cityProvider).value;
    final country = _ref.dependOn(countryProvider).value;
    return '$city ($country)';
  }
}
```
...
:::note
Depending on the provider used, it already take care of the clean-up process.  

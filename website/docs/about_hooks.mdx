...
As such, it may make sense to start first with plain Flutter/Riverpod, and comeback
...
- If we wanted, we could refactor this logic into a separate reusable function:
  ...
  `FadeIn` widget.  
  If we wanted, we could use that `useFadeIn` function is a completely different

...
For this, we will make a simple example which recommends a random brewery to users.  
...
We can make a simple application which uses [Open Brewery DB](https://www.openbrewerydb.org/)
to suggest a random brewery to users.

First, let's define a `Brewery` class:

<AutoSnippet {...activity} />

That class will be responsible for representing a suggested brewery
...
We can now use this provider to display a random brewery.  
For now, we will not handle the loading/error state, and simply
display the brewery when available:
...
This syntax can be used only because `breweryProvider` returns a non-nullable
`Brewery`.

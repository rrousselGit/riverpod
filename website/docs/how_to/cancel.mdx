...
- A detail page, which displays a random brewery from the [Open Brewery DB](https://www.openbrewerydb.org/),
  with the ability to refresh the brewery.  
...
- If the user opens the detail page and then navigates back immediately,
  we will cancel the request for the brewery.
- If the user refreshes the brewery multiple times in a row, we will debounce
...
<img
  ...
  alt="Gif showcasing the application, opening the detail page and refreshing the brewery."
/>
...
To fetch the brewery and implement pull-to-refresh, refer
...
At the moment, if the user refreshes the brewery multiple times in a row,
we will send a request for each refresh.

Technically speaking, now that we have implemented cancellation, this is not
a problem. If the user refreshes the brewery multiple times in a row,
...
the brewery for a fixed amount of time.
...
Then, if the user refreshes the brewery again before the 500ms have elapsed,

...
void main() {
  test('defaults to 0 and notify listeners when value changes', () {
    ...
    counterProvider.watchOwner(container, (value) => listener(value.state));
    ...
    counterProvider.readOwner(container).state++;
...
test('override repositoryProvider', () async {
  ...
  expect(
    todoListProvider.readOwner(container),
  ...
  expect(todoListProvider.readOwner(container).data.value, [
    isA<Todo>()
        .having((s) => s.id, 'id', '42')
...
[providerstateowner]: https://pub.dev/documentation/riverpod/latest/riverpod/ProviderContainer-class.html

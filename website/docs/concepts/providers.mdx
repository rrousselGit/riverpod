...
:::caution
While all providers have their purpose, [ChangeNotifierProvider]s are not recommended for scalable application, because of [issues with mutable state](/docs/concepts/why_immutability).
It exists in the `flutter_riverpod` package to provide an easy migration path from `package:provider`.
:::

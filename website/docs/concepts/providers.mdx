...
Technically some built-in providers could be made by just combinations of the most simple providers, but
they are made available to make common use cases more convenient.
...
While all providers have their purpose, `ChangeNotifierProviders` are not recommended for frequent use, because of // TODO: Add in link to why mutable state is bad
general issues with mutable state. It exists in the `flutter_riverpod` package to provide an easy migration path from `package:provider`.

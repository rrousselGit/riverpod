...
Scoping allows you to create new instances of the same provider in different parts of the tree, just like `package:provider` does.

However, before scoping every provider you use, consider carefully why you want to scope the provider.
In order to do so, you first need to understand what scoping means.
...
When a widget or provider requests the value of a provider, riverpod looks up the state of that provider in the [ProviderContainer] in the nearest 
...
While scopes are important to understand, it is easy to get carried away when using scopes. Here are a few questions that you might want to ask yourself.

* Why do I want a scope?
If you want multiple instances of a provider state depending on where it is in the widget tree you have a few alternatives available to you: `Scoping`, `Families`, or a combination.
The appropriate choice depends on your use case.
One major difference is that if you use a family you can potentially show multiple 'todos' in the same subtree by using family parameters to show all the ones you would like. 
If you do not use families, this requires more boilerplate in your widget tree since you have to wrap each of them in a separate `ProviderScope`.

:::warning
Remember that scopes introduce a new instance of each provider that is overridden or has listed a dependency on a provider that was overridden. 
...
* Do I have multiple providers that depend on the same provider and I want to have a new instance made when the provider they depend on is overridden?
If you have many providers that all depend on a single provider, and you want all of them to be overridden when that provider is overridden, you should use scopes. 
...
Keep it simple and clean, and use scope.

...
### Using .update() to update previous state esily

Sometimes you want to update the state based on the previous state without calling ref.read multiple times!

This is what you usually want to avoid:
...
Instead this is what you'll want to do:
...
class HomeView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          ref.read(counterProvider.notifier).update((previousCounterState) => previousCounterState + 1);
        },
      ),
    );
  }

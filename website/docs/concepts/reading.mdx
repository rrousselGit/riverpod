...
This option is for [flutter_hooks] users. Although Hooks and riverpod [Provider]s are a general replacement for
management of local state, there has been requests for a StatefulWidget version of [HookConsumerWidget].
This variant is called [StatefulHookConsumerWidget] and is a [ConsumerStatefulWidget] that allows for usage of
[flutter_hooks].
...
content from a provider whenever that content changes:
...
### Using ref.read to obtain the state of a provider at the current moment
...
:::note
Using `ref.read` should be avoided as much as possible in the widget tree (except for in callbacks)
...
:::info
It is possible to use `select` with `ref.listen` as well:

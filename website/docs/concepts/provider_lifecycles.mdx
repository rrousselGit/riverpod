...
From the other perspective, you can watch other providers to have the provider recreate itself whenever one of it's dependencies changes.
This is what makes it a reactive framework.
...
Typically when constructing a `Notifier` class you should pass in the `ref` to allow the Notifier to obtain the current value of dependencies as needed.

...
### How to update state without calling read 2 times?

Sometimes you want to update the state based on the previous state without calling ref.read multiple times!

In order to not call `ref.read` 2 times or more, you could use `ref.read().update()` by passing a callback 
function. 

You'll get the previous state as the first parameter, you should return the updated state.

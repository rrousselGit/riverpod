...
When using [watch], Riverpod is able to detect that the value being listened to
changed and will _automatically_ re-execute the provider when needed.
...
However, passing `ref.read` results in slightly less verbose code and ensures
that our object will never use `ref.watch`.
:::
...
For example, you may be listening to a `Configuration` object, but only use the
`host` property.

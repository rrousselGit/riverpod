...
Depending on the provider that you are listening to, the value obtained may change
over time.  
For example, you may be listening to a [StateNotifierProvider] and the state was changed, or the provider
listened to was forced to refresh using [ProviderContainer.refresh]/[context.refresh].
...
feature or live-configuration changes:
...
The only difference is that passing `ref.read` brings is slightly less verbose and ensures that our object never uses `ref.watch`.
:::
...
changes, this stills cause your provider to be re-evaluated – which may be
undesired.
...
final productsProvider = FutureProvider<List<Product>>((ref) async {
  ...
  final configs = await ref.watch(configProvider.last);
...
final _hostProvider = FutureProvider<String>((ref) async {
  final config = await ref.watch(configProvider.last);
  return config.host;
});
...
You can also use `.select` in simple cases which will achieve the same effect.

**PREFER** using select when a property of the object is used in one place:
...
However, if you will be using the host property in many different providers or widgets, it is better
to separate it into it's own provider.

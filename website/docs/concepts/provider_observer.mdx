...
[ProviderObserver] listens to the changes of all providers.
...
For states that are mutable such as `StateController` and `ChangeNotifier` the previousValue and newValue will be the same
since they reference the same `StateController` or `ChangeNotifier`.

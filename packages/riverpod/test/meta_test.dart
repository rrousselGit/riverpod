...
class _PublicAPIVisitor extends GeneralizingElementVisitor2<void> {
  ...
  bool _isPublicApi(Element element) {
    ...
    if (element.thisOrAncestorMatching((e) => e.metadata.hasInternal) != null) {
      return false;
    }
  ...
  void _verifyInheritsAnnotations(Element element) {
    ...
    for (final (override, className) in overrides) {
      ...
      if ((!element.metadata.hasInternal && override.metadata.hasInternal) ||
          (!element.metadata.hasProtected && override.metadata.hasProtected) ||

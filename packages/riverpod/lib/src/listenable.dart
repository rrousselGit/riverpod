...
class _Listener<T> {
  _Listener._(this.onValue, this.onError);
...
class ValueNotifier<T> extends _ChangeNotifier<T> {
  ...
  set result(Result<T>? value) {
    ...
    _result = value;
    value?.when(
      data: (newValue) => _notifyListeners(previous?.stateOrNull, newValue),
...
class _ChangeNotifier<T> {
  ...
  void Function() addListener(
    ...
    final listener = _Listener._(onChange, onError);
  ...
  void _notifyListeners(T? previous, T newValue) {
    assert(_ChangeNotifier.debugAssertNotDisposed(this), '');
    if (_count == 0) {
      return;
    }
    ...
    for (var i = 0; i < end; i++) {
      try {
        _listeners[i]?.onValue(previous, newValue);
  ...
  @protected
  @pragma('vm:notify-debugger-on-exception')
  void _notifyListenersError(Object error, StackTrace stackTrace) {
    assert(_ChangeNotifier.debugAssertNotDisposed(this), '');
    if (_count == 0) {
      return;
    }
    ...
    for (var i = 0; i < end; i++) {
      try {
        _listeners[i]?.onError?.call(error, stackTrace);
      } catch (exception, stack) {
        throw StateError(
          'An exception was thrown inside a _ChangeNotifier listener:'
          '\n$exception\n$stack',

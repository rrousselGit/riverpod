...
void main() {
  testSource(
    'Decode watch expressions with syntax errors',
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result.widgetRefWatchInvocations.single.listenable.provider,
        isNull,
      );
    },
  );
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result.widgetRefWatchInvocations[0].listenable.provider?.node
            .toSource(),
        'dep',
      );
      expect(
        result
            .widgetRefWatchInvocations[0]
            .listenable
            .provider
            ?.providerElement,
        same(
          result.manualProviderDeclarations.findByName('dep').providerElement,
        ),
      );
      ...
      expect(
        result.widgetRefWatchInvocations[1].listenable.provider?.node
            .toSource(),
        'dep2Provider',
      );
      expect(
        result
            .widgetRefWatchInvocations[1]
            .listenable
            .provider
            ?.providerElement,
        same(
      ...
      expect(
        result.widgetRefWatchInvocations[2].listenable.provider?.node
            .toSource(),
        'dep3Provider',
      );
      expect(
        result
            .widgetRefWatchInvocations[2]
            .listenable
            .provider
            ?.providerElement,
        same(
  ...
  testSource(
    'Decodes simple ref.watch usages',
    timeout: const Timeout.factor(4),
    runGenerator: true,
    source: r'''
import 'package:riverpod/riverpod.dart';
...
''',
    (resolver, unit, units) async {
      ...
      expect(result.widgetRefReadInvocations, result.widgetRefInvocations);
      expect(result.widgetRefInvocations, result.widgetRefReadInvocations);

      expect(
        result.widgetRefReadInvocations[0].node.toSource(),
        'ref.read(dep)',
      );
      ...
      expect(
        ...
        same(
          result.manualProviderDeclarations.findByName('dep2').providerElement,
        ),
      );
    },
  );
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result.widgetRefListenInvocations[0].function.toSource(),
        'listen',
      );
      expect(
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result
            .widgetRefListenManualInvocations[0]
            .listenable
            .provider
            ?.providerElement,
        same(
      ...
      expect(
        result
            .widgetRefListenManualInvocations[1]
            .listenable
            .provider
            ?.providerElement,
        same(
      ...
      expect(
        result
            .widgetRefListenManualInvocations[2]
            .listenable
            .provider
            ?.providerElement,
        same(
...
''',
  ...
  testSource(
    'Decodes family ref.watch usages',
    timeout: const Timeout.factor(4),
    runGenerator: true,
    source: r'''
import 'package:riverpod/riverpod.dart';
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result
            ...
            ?.providerElement,
        same(
          result.manualProviderDeclarations
              .findByName('family')
              .providerElement,
        ),
      );
  ...
  testSource(
    'Decodes nested ref.watch invocations with family providers',
    runGenerator: true,
    source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';
...
''',
  ...
  testSource(
    'Decodes provider.query ref.watch usages',
    timeout: const Timeout.factor(4),
    runGenerator: true,
    source: r'''
import 'package:riverpod/riverpod.dart';
...
''',
    (resolver, unit, units) async {
      ...
      expect(
        result.widgetRefWatchInvocations[3].listenable.provider?.node
            .toSource(),
        'familyProvider',
      );
      expect(
        result
            .widgetRefWatchInvocations[3]
            .listenable
            .provider
            ?.providerElement,
        same(

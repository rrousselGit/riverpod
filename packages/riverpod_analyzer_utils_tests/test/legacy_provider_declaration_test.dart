...
''',
      (resolver, unit, units) async {
        ...
        expect(result.manualProviderDeclarations, hasLength(1));
        expect(result.generatorProviderDeclarations, isEmpty);
        expect(
          result.manualProviderDeclarations.first.node.toSource(),
    ...
    testSource(
      'Does not try to parse generated providers',
      runGenerator: true,
      source: '''
import 'package:riverpod/riverpod.dart';
...
''',
      (resolver, unit, units) async {
        final result = await resolver.resolveRiverpodAnalysisResult();
        final providers = result.manualProviderDeclarations.takeAll([
          'first',
          'second',
        ]);
...
''',
      (resolver, unit, units) async {
        final result = await resolver.resolveRiverpodAnalysisResult();
        final deps = result.manualProviderDeclarations.takeAll([
          'dep',
          'family',
        ]);
        final explicitDeps = result.manualProviderDeclarations.takeAll([
          'explicitDep',
          'explicitFamily',
        ]);
        final unknownDependencies = result.manualProviderDeclarations
            .findByName('unknownDependencies');

        final generatorDependencies = result.manualProviderDeclarations
            .findByName('generatorDependencies');
        final providers = result.manualProviderDeclarations.takeAll([
          'alwaysAliveProvider',
          'alwaysAliveFamily',
        ...
        for (final provider in providers.entries) {
          ...
          expect(
            ...
            reason:
                '${provider.key} has a family expression as third dependency',
          );
          expect(
        ...
        expect(generatorDependencies.dependencies?.dependencies, hasLength(2));
        expect(
    ...
    testSource(
      'Decode LegacyProviderType.provider',
      source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod/misc.dart';
...
''',
      (resolver, unit, units) async {
        ...
        expect(providers['provider']?.build.toSource(), '(ref) => 0');

        expect(
          providers['autoDisposeFamily']?.typeArguments?.toSource(),
          '<int, int>',
        );
        expect(providers['provider']?.typeArguments?.toSource(), '<int>');
    ...
    testSource(
      'Decode LegacyProviderType.futureProvider',
      source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod/misc.dart';
...
''',
    ...
    testSource(
      'Decode LegacyProviderType.stateProvider',
      source: '''
...
''',
    ...
    testSource(
      'Decode LegacyProviderType.streamProvider',
      source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod/misc.dart';
...
''',
    ...
    testSource(
      'Decode LegacyProviderType.notifierProvider',
      source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod/misc.dart';

final alwaysAliveProvider = NotifierProvider<Notifier<int>, int>(() => throw UnimplementedError());
final alwaysAliveFamily = NotifierProvider.family<Notifier<int>, int, int>((arg) => throw UnimplementedError());
final autoDisposeProvider = NotifierProvider.autoDispose<Notifier<int>, int>(() => throw UnimplementedError());
final autoDisposeFamily = NotifierProvider.autoDispose.family<Notifier<int>, int, int>((arg) => throw UnimplementedError());
final autoDisposeFamily2 = NotifierProvider.family.autoDispose<Notifier<int>, int, int>((arg) => throw UnimplementedError());
''',
      (resolver, unit, units) async {
        final result = await resolver.resolveRiverpodAnalysisResult();
        final providers = result.manualProviderDeclarations.takeAll([
    ...
    testSource(
      'Decode LegacyProviderType.asyncNotifierProvider',
      source: '''
import 'package:riverpod/riverpod.dart';
import 'package:riverpod/misc.dart';

final alwaysAliveProvider = AsyncNotifierProvider<AsyncNotifier<int>, int>(() => throw UnimplementedError());
final alwaysAliveFamily = AsyncNotifierProvider.family<AsyncNotifier<int>, int, int>((arg) => throw UnimplementedError());
final autoDisposeProvider = AsyncNotifierProvider.autoDispose<AsyncNotifier<int>, int>(() => throw UnimplementedError());
final autoDisposeFamily = AsyncNotifierProvider.autoDispose.family<AsyncNotifier<int>, int, int>((arg) => throw UnimplementedError());
final autoDisposeFamily2 = AsyncNotifierProvider.family.autoDispose<AsyncNotifier<int>, int, int>((arg) => throw UnimplementedError());
''',
      (resolver, unit, units) async {
        final result = await resolver.resolveRiverpodAnalysisResult();
        final providers = result.manualProviderDeclarations.takeAll([
    ...
    testSource(
      'Decode LegacyProviderType.changeNotifierProvider',
      source: '''
...
''',
    ...
    testSource(
      'Decode LegacyProviderType.stateNotifierProvider',
      source: '''
import 'package:riverpod/legacy.dart';
import 'package:flutter_riverpod/misc.dart';
...
''',
      (resolver, unit, units) async {
        ...
        final families = result.manualProviderDeclarations.takeAll([
          'alwaysAliveFamily',
          'explicitAlwaysAliveFamily',
          'autoDisposeFamily',
          'autoDisposeFamily2',

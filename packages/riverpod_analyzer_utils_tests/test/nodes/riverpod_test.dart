...
void main() {
  testSource(
    'Decode dependencies with syntax errors',
    source: '''
...
''',
    (resolver, unit, units) async {
      ...
      expect(errors[0].message, 'Only list literals (using []) as supported.');
      expect(errors[0].targetNode.toSource(), 'deps');

      expect(errors[1].message, 'Only list literals (using []) as supported.');
      ...
      expect(errors[3].message, 'if/for/spread operators as not supported.');
...
''',
    (resolver, unit, units) async {
      expect(unit.declarations.findByName('unrelated').riverpod, isNull);
